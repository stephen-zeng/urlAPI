...
function getValue(tasks) {
  ...
  for (let task of tasks) {
    if (task.status in map.value) {
      map.value[task.status]++;
    } else {
      map.value[task.status] = 1;
    }
  }
...
<template>
  <mdui-collapse>
    <mdui-collapse-item rounded>
      <mdui-list-item slot="header" icon="done" rounded @click="getValue(props.tasks)" :disabled="!props.fetched">
        状态
