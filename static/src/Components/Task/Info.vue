...
function getValue(tasks) {
  ...
  for (let task of tasks) {
    if (task.more_info in map.value) {
      map.value[task.more_info]++;
    } else {
      map.value[task.more_info] = 1;
    }
  }
...
<template>
  <mdui-collapse>
    <mdui-collapse-item rounded>
      <mdui-list-item slot="header" icon="info" rounded @click="getValue(props.tasks)" :disabled="!props.fetched">
        信息
