...
function getValue(tasks) {
  ...
  for (let task of tasks) {
    if (task.device in map.value) {
      map.value[task.device]++;
    } else {
      map.value[task.device] = 1;
    }
  }
...
<template>
  <mdui-collapse>
    <mdui-collapse-item rounded>
      <mdui-list-item slot="header" icon="devices" rounded @click="getValue(props.tasks)" :disabled="!props.fetched">
        设备
