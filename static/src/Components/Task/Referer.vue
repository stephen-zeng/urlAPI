...
function getValue(tasks) {
  ...
  for (let task of tasks) {
    if (task.referer in map.value) {
      map.value[task.referer]++;
    } else {
      map.value[task.referer] = 1;
    }
  }
...
<template>
  <mdui-collapse>
    <mdui-collapse-item rounded>
      <mdui-list-item slot="header" icon="source" rounded @click="getValue(props.tasks)" :disabled="!props.fetched">
        来源
